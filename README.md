...
1. **Siapkan file environment untuk Docker**
   ```bash
   cp .env.docker.example .env
   ```
   Jika Anda memakai MongoDB Atlas, ganti `MONGODB_URI` sesuai URI Atlas Anda.

2. **Pilih mode database**
- **Pakai MongoDB Atlas/eksternal**: isi `MONGODB_URI` ke URI Atlas Anda.
- **Pakai MongoDB lokal dari Docker**: biarkan nilai default `mongodb://mongo:27017/whatsapp-webhook`.

3. **Build dan jalankan container**
   - Mode Atlas/eksternal:
   ```bash
   docker compose up -d --build app
   ```
   - Mode Mongo lokal Docker:
   ```bash
...
Catatan:
- Session WhatsApp disimpan persisten di folder `auth_info_baileys`.
- Log aplikasi disimpan persisten di folder `logs`.
- Data MongoDB disimpan dalam volume Docker `mongo_data`.

---

## Menjalankan Dengan PM2

1. **Install PM2 (global, sekali saja)**
   ```bash
   npm install -g pm2
   ```

2. **Pastikan `.env` sudah benar**
- `MONGODB_URI` mengarah ke MongoDB Anda (lokal atau Atlas).
- `SESSION_SECRET` sudah diisi.

3. **Jalankan app dengan PM2**
   ```bash
   npm run pm2:start
   ```

4. **Perintah operasional**
   ```bash
   npm run pm2:logs
   npm run pm2:restart
   npm run pm2:stop
...
5. **Agar auto-start saat server reboot**
   ```bash
   npm run pm2:startup
   npm run pm2:save
   ```
...
Catatan:
- Konfigurasi PM2 ada di `ecosystem.config.cjs`.
- App dijalankan mode `fork` (1 instance) agar sesi WhatsApp tidak bentrok.
