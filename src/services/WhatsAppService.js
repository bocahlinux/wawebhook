...
class WhatsAppService {
    constructor(io) {
        ...
        this.creatingSessions = new Map(); // userId => Promise<session>
    ...
    async ensureSession(userId, phoneNumber = null) {
        ...
        if (this.creatingSessions.has(userIdStr)) {
            return await this.creatingSessions.get(userIdStr);
        }

        const createPromise = this.createSession(userIdStr, phoneNumber)
            .finally(() => this.creatingSessions.delete(userIdStr));
        this.creatingSessions.set(userIdStr, createPromise);

        return await createPromise;
    }

    getBrowserSignature() {
        const profile = String(config.whatsapp.browserProfile || 'macos').toLowerCase();
        const browserName = config.whatsapp.browserName || 'Google Chrome';

        switch (profile) {
            case 'ubuntu':
                return Browsers.ubuntu(browserName);
            case 'windows':
                return Browsers.windows(browserName);
            case 'mac':
            case 'macos':
            default:
    ...
    async createSession(userId, phoneNumber = null) {
        ...
        const sock = makeWASocket({
            ...
            browser: this.getBrowserSignature(),
    ...
    handleConnectionUpdate(update, session, userId, authDir) {
        ...
        if (connection === 'close') {
            ...
            if (statusCode === DisconnectReason.loggedOut || statusCode === 405) {
                const reasonText = statusCode === 405 ? 'Connection Failure (405)' : 'Intentional logout';
                console.log(`${reasonText} detected for user ${userId}; clearing session.`);
                try {
            ...
            // Reconnect for all non-logout closures with controlled backoff
            if (shouldReconnect) {
                const delay = this.getReconnectDelay(userId, statusCode);
                console.log(`Reconnecting for user ${userId} in ${delay}ms...`);
                this.scheduleReconnect(userId, delay);
            } else {
    ...
    getReconnectDelay(userId, statusCode) {
        if (statusCode === DisconnectReason.restartRequired) return 0;

        const prevAttempts = this.reconnectAttempts.get(userId) || 0;
        const nextAttempts = prevAttempts + 1;
        this.reconnectAttempts.set(userId, nextAttempts);

        // Error 405 can be temporary rejection. Avoid reconnect storm by exponential backoff.
        if (statusCode === 405) {
            const base = 15000; // 15s
            const capped = Math.min(base * Math.pow(2, Math.max(0, nextAttempts - 1)), 120000); // max 2m
            return capped;
        }

        return Math.min(3000 * Math.pow(2, Math.max(0, nextAttempts - 1)), 30000); // max 30s
    }

    scheduleReconnect(userId, delayMs) {
        ...
        const timer = setTimeout(() => {
            this.reconnectTimers.delete(userId);
            this.ensureSession(userId).catch((err) => {
                console.error(`Reconnect failed for user ${userId}:`, err);
            });
        }, delayMs);
    ...
    async logout(userId) {
        ...
        const reconnectTimer = this.reconnectTimers.get(userIdStr);
        if (reconnectTimer) {
